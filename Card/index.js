...
CardApp.post(
  ...
  async (req, res) => {
    ...
    console.log(id);
    const { name, phone, father, district, dob, address, aadhar_no } = req.body;
...
CardApp.get("/all/:page", authenticateToken, async (req, res) => {
  ...
  const val = await CardSchema.find({})
    .limit(50)
    .skip(page * 50)
    .sort({ created: -1 });
...
CardApp.get("/search/:text", authenticateToken, async (req, res) => {
  ...
  const val = await CardSchema.find({ id: new RegExp(text, "i") }).sort({
    created: -1,
  });
